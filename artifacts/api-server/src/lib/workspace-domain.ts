...
MANDATORY for ALL Gabriel visualization types (not just engagement dashboards):
  ...
  • **Nav menus / sidebars:** Wrap nav + sections in \`data-viz-nav-root\`. Put \`data-viz-nav\` on the nav, \`data-viz-nav-item="id"\` on links, \`data-viz-section="id"\` on sections. Script scopes to the nearest \`[data-viz-nav-root]\` so multiple navs on one page never collide. Click reveals matching section, hides others, toggles viz-nav-active class. e.preventDefault() on anchors.
