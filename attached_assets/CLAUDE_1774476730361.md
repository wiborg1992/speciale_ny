...
Browser (index.html)
  ...
  │     ├── Web Speech API (Browser, da-DK / en-US)
  │     └── Deepgram Nova-2 (Meeting-tilstand, multi-sprog)
  │           └── /api/deepgram-token → nøgle fra server (.env)
  │
...
server.js (Node.js, ingen frameworks)
  ...
  ├── GET  /api/deepgram-token   → Deepgram API-nøgle til browser WebSocket
...
DEEPGRAM_API_KEY=...    # Valgfri — Meeting-tilstand (Deepgram streaming)
PORT=3000
```
...
| Transskription | Web Speech API + Deepgram Nova-2 | Meeting kræver DEEPGRAM_API_KEY |
...
### REGEL 2: Deepgram Meeting
- Kræver `DEEPGRAM_API_KEY` i `.env`; token udleveres via `GET /api/deepgram-token` (browser bruger WebSocket subprotocol `token`).
- `sample_rate` i query skal matche faktisk AudioContext-rate (app’en sætter det dynamisk).
...
Konstant: AUTO_VIZ_INTERVAL_MS = 45000 (45 sekunder, i index.html)

Flow:
1. startAutoVizInterval() starter et setInterval ved optagelsesstart
2. Hvert 45. sekund: triggerVisualize(isAuto = true)
3. Sender { isIncremental: true, previousViz: currentVizHtml } til server
4. Claude opdaterer eksisterende prototype — starter ikke forfra
5. Timer nulstilles ved hver viz (auto eller manuel)
...
4. Eventuelle forbedringer af Deepgram / Web Speech-kvalitet
...
## Hurtig Start (lokal dev)
  ...
  cd specialev-rkt-j
  cp .env.example .env
  # Udfyld ANTHROPIC_API_KEY (og valgfrit DEEPGRAM_API_KEY til Meeting)
